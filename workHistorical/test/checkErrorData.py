...
def checkErr(initial_date, end_date, market):
    try:
        ...
        # creiamo la cartella error se non esiste
        error_path = Path(f'{project_root}/workHistorical/test/error')
        if not os.path.exists(error_path):
            os.makedirs(error_path)
        
        
        with open(f'{project_root}/workHistorical/test/error/errorData_{market}.txt', 'a') as file_out:
...
"""
Si analizza la variazione giornaliera dei prezzi e si confronta con una soglia. 
"""
def detect_anomalies_same_and_next_day(data, threshold_abs=1000, threshold_pct=30):
    """
    data: lista di tuple (time_value, open_price, high_price).
          Si assume sia ORDINATA per data.
    ...
    Ritorna: lista di dizionari con i dettagli delle anomalie rilevate
    """
    ...
    for i in range(len(data) - 1):
        ...
        #anomaly_same_day = (diff_same_day > threshold_abs) or (diff_same_day_pct > threshold_pct)
        #anomaly_next_day = (diff_next_day > threshold_abs) or (diff_next_day_pct > threshold_pct)
