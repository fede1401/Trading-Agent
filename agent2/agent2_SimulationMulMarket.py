    ...
    try:
        ...
        for m in market:
            ...
            for i in range(100):
                ...
                # trading per 1 anno
                profitto1Y = tradingYear(cur, conn, symbols, trade_date, m, initial_date, endDate)
                
                # Inserimento dei dati relativi al profitto dell'agente nel database
                insertDataDB.insertInTesting(idTest, "agent2", i, initial_date=initial_date, end_date=endDate, profit=profitto1Y, market=m, notes=f"trading sui 100 simboli azionari del mercato {m} per capitalizzazione decrescente ",  cur=cur, conn=conn)
...
def tradingYear(cur, conn, symbols, trade_date, market, initial_date, endDate):    
